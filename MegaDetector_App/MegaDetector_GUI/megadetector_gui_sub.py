...
def run_commands_thread():
    ...
    for subdir in os.listdir(base_image_folder):
        ...
        if os.path.isdir(subdir_path):
            ...
            commands = [
                f'{python_executable} C:\Users\Public\Documents\MegaDetector_App\MegaDetector\megadetector\detection\run_detector_batch.py "{model_path}" "{subdir_path}" "{json_output_path}" --class_mapping_filename "{class_mapping_filename}" --output_relative_filenames --recursive --checkpoint_frequency 1000 --quiet',
                f'{python_executable} C:\Users\Public\Documents\MegaDetector_App\MegaDetector\megadetector\api\batch_processing\postprocessing\postprocess_batch_results.py "{json_output_path}" "{subdir_output_path}" --image_base_dir "{subdir_path}" --num_images_to_sample -1'
            ]
